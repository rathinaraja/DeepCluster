...
def main(): 
    ...
    if args.gpu_ids is not None:
        ...
        # FIXED: Validate GPU IDs before trying to access them
        if torch.cuda.is_available():
            ...
            if valid_gpu_ids:
                ...
                gpu_ids = valid_gpu_ids  # Use only valid GPU IDs
            else:
        ...
        if gpu_ids:  # Only proceed with GPU processing if we have valid GPUs
    ...
    # All GPUs     
        ...
        if gpu_ids:  # Only proceed with GPU processing if we have GPUs
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            process_all_input_folders_parallel(config, device, log_file, gpu_ids)
        else:
            # Fall back to CPU processing
