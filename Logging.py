    ...
    if config.sub_folders:
        ...
        with open(log_file, mode='a', newline='') as file:
            ...
            if not file_exists:
                ...
                header.extend(["total_samples", "num_clusters", "num_samples_selected", "feature_extraction_time(s)", "clustering_time(s)", "sampling_time(s)", "overall_time(s)", "date_time_processed", "additional_note"])
        ...
        with open(log_file, mode='a', newline='') as file:
            ...
            # If the file does not exist, write the header
            if not file_exists:      
                writer.writerow(["WSI_Name", "total_samples", "num_clusters", "num_samples_selected", "feature_extraction_time(s)", "clustering_time(s)", "sampling_time(s)", "overall_time(s)", "date_time_processed", "additional_note"]) 

# Here lock is multiprocessing object
def write_metrics_to_log(log_file, folder_name, metrics_dict, csv_lock):
    ...
    try:
        with csv_lock:
            with open(log_file, mode='a', newline='') as file:
                ...
                row_data.extend([
                    metrics_dict.get('total_samples', 0),
                    metrics_dict.get('num_clusters', 0),
                    metrics_dict.get('num_samples_selected', 0),
                    metrics_dict.get('feature_extraction_time', 0.0),
                    metrics_dict.get('clustering_time', 0.0),
                    metrics_dict.get('sampling_time', 0.0),
                    metrics_dict.get('overall_time', 0.0),
                    metrics_dict.get('date_time_processed', ''),
                    metrics_dict.get('additional_note', '')
                ])
