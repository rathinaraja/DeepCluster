...
class ProvGigaPathEncoder:
    """
    Prov-GigaPath tile-encoder via HF Hub.
    Config:
      - Config.prov_repo_id: e.g. "prov-gigapath/prov-gigapath" (set by you)
      - Config.prov_filename: e.g. "pytorch_model.bin" (fallback load)
    """
    ...
    def _build_model_and_transform(self):
        repo_id = getattr(Config, "prov_repo_id", "")
        filename = getattr(Config, "prov_filename", "pytorch_model.bin")

        # 1) Try direct timm HF-hub loading
        if repo_id:
            try:
                m = timm.create_model(
                    f"hf-hub:{repo_id}", pretrained=True, num_classes=0, dynamic_img_size=True
                ).to(self.device).eval()
                cfg = resolve_data_config(m.pretrained_cfg, model=m)
                return m, create_transform(**cfg)
            except Exception as e:
                print(f"[Prov-GigaPath] Direct timm HF load failed: {e}. Falling back to manual checkpoint...")

        # 2) Fallback onto ViT-B/16 backbone (adjust if your ckpt uses a different arch)
        m = timm.create_model("vit_base_patch16_224", pretrained=False, num_classes=0).to(self.device).eval()

        if repo_id:
            ckpt_path = hf_hub_download(repo_id=repo_id, filename=filename)
        else:
            raise RuntimeError("Set Config.prov_repo_id to a valid HF repo id for Prov-GigaPath.")
    ...
    def get_encoder_name(self): return "prov_gigapath"
